...
// Defends against exposed passwords in use or in transit like how hashing defends against exposed passwords at rest.

// Instead of sending the password directly, the client first derives an asymmetric key pair from the password using a
// key derivation function (KDF). The client then obtains a random challenge from the server, signs the challenge with
// its private key and sends the signature and public key back to the server. The server verifies the signature matches
// the public key. If so, normal logon or registration continues, treating the public key similar to a password.

// This example server is a simple HTTP server that implements the above protocol. Data is stored in the filesystem for
// simplicity. Challenges are stored in memory and removed after use. /register checks to see if the name has already
// been registered and if not, creates a new folder whose name is the hex encoding of the provided username. Inside the
// folder, it stores a file called "salt" containing a random salt, and a file called "hmac" containing the HMAC of the
// public key and salt. Salting and hashing the public key hinders brute force and precomputation attacks in the event
// that the data at rest (hash database) is leaked. This is similar to normal password storage except that only one
// iteration is required on the server, since the client has already done many hash iterations using the KDF.
...
pub fn main() {
	rouille::start_server("127.0.0.1:2203", move |request| {
		...
		if request.raw_url() == "/challenge" {
			...
			CHALLENGES.lock().ok().map(|mut set| set.insert(challenge.clone())); //save challenge
		...
		if let Ok(challenge_array) = <&[u8; 32]>::try_from(challenge)
				&& ! CHALLENGES.lock().map(|mut set| set.remove(challenge_array)).unwrap_or(false){
		...
		let name = if let Some(n) = request.header("name") && n.len() < 64 && n.len() > 0 {
			n
		} else {
		...
		if request.raw_url() == "/register" {
			...
			if std::fs::create_dir_all(&namehex).is_err()
				...
				if let Err(e) = std::fs::remove_dir_all(&namehex){ //on error, try to undo everything
			...
			if let Ok(salt) = std::fs::read(format!("{}/salt",	 &namehex))
				...
				if hmac.verify_slice(&hmac_bytes).is_err(){
					return Response::text("Invalid credentials").with_status_code(401); //This is what a bad password will normally return
				}
